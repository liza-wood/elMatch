---
title: "elMatch-vignette"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{elMatch-vignette}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
```

```{r setup}
#devtools::install_github('liza-wood/elMatch')
library(elMatch)
```

Pull the list of projects

Pull in the applicant data

Read in a sample transcript
```{r}
student_url <- 'https://docs.google.com/spreadsheets/d/1g6y2zkolHAOrtppdSThsBkRsl3RCVa7_Nve_NEOQARc/edit?usp=sharing'
project_url <- 'https://docs.google.com/spreadsheets/d/1oJd2JNKZ7CLTpwexepS7Ly1f19oLchWji6yRxz7X3X8/edit?resourcekey#gid=1824769055'
p_mat <- pull_research_needs(project_url, student_url)
s_mat <- pull_student_interest(student_url)

proj_list <- identify_top_matches(p_mat, s_mat, cutoff = 1)
```

Take a look at the project match
```{r}
proj_list[2]
```

```{r}
p_index <- get_project_index(project_url)
s_index <- get_student_index(student_url)
```

Use this function to send further assessments based on surveys. This will update the student index so that students aren't emailed a million times. 
```{r, eval = F}
email_assessments(sender = 'belwood@ucdavis.edu')
```

