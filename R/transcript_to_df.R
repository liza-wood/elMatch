...
transcript_to_df <- function(file){
  ...
  grades <- paste0(" ", paste(c(LETTERS[c(1:4,6,9,19)],
                                ...
                                paste0(LETTERS[c(1:4,6)], "\\-")),
                              collapse = " | "), " ")
  ext <- str_extract_all(t, paste0("(", subj_code, ")", ".*", "(",grades, ")"))
  df <- data.frame('raw' = ext[[1]])

  df$subj_code <- str_extract(df$raw,
                              paste0('^(',str_remove_all(subj_code, '\\s'), ')'))
  df$raw_cut <- trimws(str_remove_all(df$raw,
                                      paste0('^(',str_remove_all(subj_code, '\\s'), ')')))
  df$course_num <- str_extract(df$raw_cut, '^[0-9A-Z]{3,5}')
  df$raw_cut <- trimws(str_remove_all(df$raw_cut, '^[0-9A-Z]{3,5}'))
  df$grade <- str_extract(df$raw_cut,
                          paste0('(',str_remove_all(grades, '\\s'), ')$'))
  df$course_name <- trimws(str_remove_all(df$raw_cut,
                                          paste0('(',str_remove_all(grades, '\\s'), ')$')))
